...
title_ratings = load_tsv(tsv_files["title_ratings"], "Title Ratings")
...
status_update("Merging ratings into filtered movies...", "⭐")
filtered_titles = filtered_titles.merge(
    title_ratings,
    on="tconst",
    how="left"
)
status_update(f"Added ratings for {filtered_titles['averageRating'].notnull().sum()} movies.", "✅")
